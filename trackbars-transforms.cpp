...
void warping1b(Mat warp1b, Mat original) {
...
int main() {
	...
	while (1) {
		if (GetKeyState('S') & 0x8000) {
			Mat interp(int(original.rows*Cy), int(original.cols*Cx), CV_8UC1, Scalar(0));
			Mat trasladada = interp.clone();
			Mat rotada = trasladada.clone();
			Mat sesH = rotada.clone();
			Mat sesV = sesH.clone();

			escalamiento(Cx, Cy, interp, original);
			traslacion(Tx, Ty,trasladada, interp);
			rotacion(angulo, rotada, trasladada);
			sesgadoH(factorSx, sesH, rotada);
			sesgadoV(factorSy, sesV, sesH);
